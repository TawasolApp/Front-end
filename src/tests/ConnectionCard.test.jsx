...
describe("ConnectionCard", () => {
  ...
  it("renders correctly with props", () => {
    ...
    expect(screen.getByText(`${mockProps.firstName} ${mockProps.lastName}`)).toBeInTheDocument();
  ...
  it("uses default profile picture when imageUrl is not provided", () => {
    ...
    const image = screen.getByAltText(`${mockProps.firstName} ${mockProps.lastName}`);
  ...
  it("navigates to user profile when name is clicked", () => {
    ...
    const nameElement = screen.getByText(`${mockProps.firstName} ${mockProps.lastName}`);
  ...
  it("closes confirmation modal with X button", () => {
    ...
    const xButton = screen.getByText(/Remove Connection/).parentElement.querySelector("button");
  ...
  it("correctly displays full name in confirmation modal", () => {
    ...
    const modalText = screen.getByText(
      new RegExp(`Are you sure you want to remove ${mockProps.firstName} ${mockProps.lastName} as a connection`, 'i')
    );
    expect(modalText).toBeInTheDocument();
  });
});