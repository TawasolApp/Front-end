...
vi.mock('@mui/material', () => ({
  Avatar: ({ children, src, alt, sx }) => (
    <div className="mock-avatar" data-testid="profile-avatar" data-src={src} data-alt={alt}>
      {children}
    </div>
  )
}));
...
describe('PeopleSearch', () => {
  ...
  it('displays headline information correctly', async () => {
    ...
    expect(screen.getByText('Product Manager at Microsoft')).toBeInTheDocument();
    expect(screen.getByText('Frontend Developer at LinkedIn')).toBeInTheDocument();
  });
  ...
  it('handles API error correctly', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Failed to load people. Please try again.')).toBeInTheDocument();
    });
  ...
  it('loads more people when page changes', async () => {
    ...
    const firstPagePeople = Array(10).fill(null).map((_, i) => ({
      userId: `user${i}`,
      firstName: `First${i}`,
      lastName: `Last${i}`,
      profilePicture: `profile-${i}.jpg`,
