...
describe("CompanySearch", () => {
  ...
  it("shows verification badge for verified companies", async () => {
    ...
    const linkedInContainer =
      screen.getByText("LinkedIn").closest(".flex-justify-between") ||
  ...
  it("handles company description display correctly", async () => {
    ...
    expect(
      screen.getByText("Search engine and tech company"),
    ).toBeInTheDocument();
    expect(screen.getByText("Software and cloud company")).toBeInTheDocument();
    expect(
      screen.getByText("Professional networking platform"),
    ).toBeInTheDocument();
  });
