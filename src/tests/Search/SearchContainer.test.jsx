...
vi.mock("../../pages/Search/SearchCompany", () => ({
  default: () => (
    <div data-testid="search-companies">Search Companies Component</div>
  ),
}));
...
describe("SearchContainer", () => {
  ...
  it("toggles the filter dropdown when button is clicked", () => {
    ...
    expect(
      screen.getByRole("menuitem", { name: "People" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("menuitem", { name: "Companies" }),
    ).toBeInTheDocument();
  ...
  it("changes the selected filter and renders appropriate component", () => {
    ...
    expect(
      screen.queryByRole("menuitem", { name: "Posts" }),
    ).not.toBeInTheDocument();
