...
vi.mock('../../../pages/Feed/GenericComponents/ActorHeader', () => ({
  default: ({ authorId, authorName, authorBio, authorPicture, iconSize, enableLink }) => (
...
describe('DropdownUsers Component', () => {
  ...
  it('does not render anything when users array is empty', () => {
    ...
    const { container } = renderWithRouter(<DropdownUsers name="test" onSelect={() => {}} />);
  ...
  it('fetches and renders users when name prop has value', async () => {
    ...
    expect(screen.getAllByTestId('author-name')[0]).toHaveTextContent('John Doe');
    expect(screen.getAllByTestId('author-bio')[0]).toHaveTextContent('Software Engineer');
  });
  ...
  it('clears users when name prop is empty', async () => {
    ...
    const { rerender } = renderWithRouter(<DropdownUsers name="jo" onSelect={() => {}} />);
    ...
    rerender(<BrowserRouter><DropdownUsers name="" onSelect={() => {}} /></BrowserRouter>);
  ...
  it('passes correct props to ActorHeader', async () => {
    ...
    expect(screen.getAllByTestId('author-name')[0]).toHaveTextContent('John Doe');
    expect(screen.getAllByTestId('author-bio')[0]).toHaveTextContent('Software Engineer');
    expect(screen.getAllByTestId('author-picture')[0]).toHaveTextContent('john.jpg');
