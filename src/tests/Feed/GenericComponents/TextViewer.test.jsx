...
describe('TextViewer Component', () => {
  describe('processText function', () => {
    ...
    it('handles multiple mentions correctly', () => {
      ...
      expect(firstMention.closest('a')).toHaveAttribute('href', '/users/user123');
      
      // Check second mention
      const secondMention = screen.getByText("Jane Smith");
      expect(secondMention).toBeInTheDocument();
      expect(secondMention.closest('a')).toHaveAttribute('href', '/users/user456');
    ...
    it('handles multiple URLs correctly', () => {
      ...
      expect(urlLinks[0].closest('a')).toHaveAttribute('href', 'https://example.com');
      expect(urlLinks[1].closest('a')).toHaveAttribute('href', 'https://test.com');
    });

    it('processes mixed mentions and URLs correctly', () => {
      const text = "@**John Doe** shared https://example.com with @**Jane Smith**";
      const taggedUsers = ["user123", "user456"];
    ...
    it('processes http and https URLs correctly', () => {
      ...
      const urlLinks = screen.getAllByText("External Link");
      expect(urlLinks).toHaveLength(2);
      expect(urlLinks[0].closest('a')).toHaveAttribute('href', 'http://example.com');
      expect(urlLinks[1].closest('a')).toHaveAttribute('href', 'https://secure.com');
    });
  ...
  describe('truncation logic', () => {
    it('truncates text when it exceeds maxChars', () => {
      const longText = "This is a very long text that should be truncated because it exceeds the maximum character limit set for the component.";
    ...
    it('truncates mentions correctly when they would exceed maxChars', () => {
      const text = "Start @**John Doe** is mentioned near the truncation point.";
      const taggedUsers = ["user123"];
      
      renderWithRouter(<TextViewer text={text} taggedUsers={taggedUsers} maxChars={20} />);
      ...
      expect(mention).toBeInTheDocument();
      expect(mention.closest('a')).toHaveAttribute('href', '/users/user123');
      expect(screen.getByText("is mentioned near the truncation point.")).toBeInTheDocument();
    });

    it('truncates URLs correctly when they would exceed maxChars', () => {
      const text = "Start https://example.com is a URL near the truncation point.";
      ...
      expect(urlLink).toBeInTheDocument();
      expect(urlLink.closest('a')).toHaveAttribute('href', 'https://example.com');
      expect(screen.getByText("is a URL near the truncation point.")).toBeInTheDocument();
    });
  });
