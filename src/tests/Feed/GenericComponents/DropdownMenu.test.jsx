...
describe("DropdownMenu Component", () => {
  ...
  it("renders with custom position", () => {
    render(
      <DropdownMenu menuItems={testMenuItems} position="left-0 top-0">
        <button data-testid="trigger-button">Open Menu</button>
      </DropdownMenu>,
    );
    ...
    const menuContainer = screen
      ...
      .closest("div").parentElement; // Find the div containing the buttons (p-1) // Find the outer div which has our position classes

    // Now check the classes
    expect(menuContainer.className).toContain(
      "w-64 bg-cardBackground rounded-lg border border-cardBorder shadow-lg ",
    );
    expect(menuContainer.className).toContain(
      "w-64 bg-cardBackground rounded-lg border border-cardBorder shadow-lg ",
    );
  ...
  it("renders with custom width", () => {
    render(
      <DropdownMenu menuItems={testMenuItems} width="w-96">
        <button data-testid="trigger-button">Open Menu</button>
      </DropdownMenu>,
    );
  ...
  it("applies custom icon size", () => {
    render(
      <DropdownMenu menuItems={testMenuItems} iconSize="w-8 h-8">
        <button data-testid="trigger-button">Open Menu</button>
      </DropdownMenu>,
    );
