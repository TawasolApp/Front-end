...
describe("TextModal Component", () => {
  ...
  it("uploads media and renders preview", async () => {
    ...
    expect(
      document.querySelector(".mt-4.border.border-gray-200"),
    ...
    await waitFor(() => {
      ...
      const mediaContainer = document.querySelector(
        ".mt-4.border.border-gray-200",
      );
      expect(mediaContainer).toBeInTheDocument();

      // Find all images and check if our image is present
      const allImages = screen.getAllByRole("img");
      const uploadedImage = Array.from(allImages).find((img) =>
        img.src.includes("cdn.example.com/image/test.png"),
      );
