...
vi.mock("@mui/material/Avatar", () => ({
  default: ({ sx, className, src }) => (
    <div data-testid="avatar" className={className} style={sx} data-src={src}>
      Mock Avatar
    </div>
  ),
}));
...
vi.mock("../../../../pages/Feed/MainFeed/SharePost/TextModal", () => ({
  default: ({
    ...
    <div data-testid="text-modal">
      ...
      <img
        data-testid="modal-author-picture"
        src={currentAuthorPicture}
        alt="Profile"
      />
      <button data-testid="modal-close" onClick={setIsModalOpen}>
        Close
      </button>
      <button
...
describe("SharePost", () => {
  ...
  it('opens modal when clicking the "Start a post" button', () => {
    ...
    expect(screen.getByTestId("modal-author-name").textContent).toBe(
      "John Doe",
    );
    expect(screen.getByTestId("modal-author-picture").getAttribute("src")).toBe(
      "profile-pic.jpg",
    );
  });
