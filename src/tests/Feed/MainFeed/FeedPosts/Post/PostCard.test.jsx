...
vi.mock('react-router-dom', () => ({
  ...
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/Post/Content/MediaContent/PdfViewer', () => ({
  ...
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/Post/Header/PostCardHeader', () => ({
    default: ({ menuItems, modal, noRightItems }) => (
      <div data-testid="post-card-header">
        {!noRightItems && menuItems && menuItems.map((item, index) => (
          <button key={index} onClick={item.onClick} data-testid={`menu-item-${item.text.replace(/\s+/g, '-').toLowerCase()}`}>
            {item.text}
          </button>
        ))}
  ...
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/Post/Content/PostContent', () => ({
    default: ({ modal, handleOpenPostModal, reposted }) => (
      <div data-testid={`post-content${reposted ? '-reposted' : ''}`}>
        <button data-testid="open-modal-button" onClick={() => handleOpenPostModal(0)}>
          Open Modal
        </button>
  ...
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/Post/Metrics/EngagementMetrics', () => ({
    default: ({ setShowLikes, setShowComments }) => (
      <div data-testid="engagement-metrics">
        <button data-testid="show-likes-button" onClick={setShowLikes}>Show Likes</button>
        <button data-testid="show-comments-button" onClick={setShowComments}>Show Comments</button>
      </div>
    )
  }));
  
  // Fix the path for ActivitiesHolder
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/Post/Activities/ActivitiesHolder', () => ({
    default: ({ setShowComments }) => (
      <div data-testid="activities-holder">
        <button data-testid="show-comments-activity" onClick={setShowComments}>Comment</button>
      </div>
    )
  }));
  
  // Fix the path for CommentsContainer
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/Post/Comments/CommentsContainer', () => ({
  ...
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/ReactionModal/ReactionsModal', () => ({
    default: ({ setShowLikes }) => (
      <div data-testid="reactions-modal">
        <button data-testid="close-likes" onClick={setShowLikes}>Close</button>
      </div>
    )
  }));
  ...
  vi.mock('../../../../../pages/Feed/MainFeed/SharePost/TextModal', () => ({
    default: ({ setIsModalOpen }) => (
      <div data-testid="text-modal">
        <button data-testid="close-edit-modal" onClick={setIsModalOpen}>Close</button>
      </div>
    )
  }));
  ...
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/DeleteModal/DeletePostModal', () => ({
    default: ({ closeModal, deleteFunc, commentOrPost }) => (
      <div data-testid="delete-modal">
        ...
        <button data-testid="close-delete-modal" onClick={closeModal}>Cancel</button>
        <button data-testid="confirm-delete" onClick={deleteFunc}>Delete</button>
      </div>
    )
  }));
  
  // Fix path for SilentRepostHeader
  vi.mock('../../../../../pages/Feed/MainFeed/FeedPosts/Post/Header/SilentRepostHeader', () => ({
  ...
  it('does not show edit options for non-owners/non-admins', () => {
    ...
    expect(screen.queryByTestId('menu-item-delete-post')).not.toBeInTheDocument();
  });
