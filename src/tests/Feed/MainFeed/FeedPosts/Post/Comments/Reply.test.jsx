...
vi.mock('../../../../../../pages/Feed/MainFeed/FeedPosts/Post/Comments/ActivitiesHolder', () => ({
  default: ({ handleReaction, setShowReactions }) => (
    <div data-testid="activities-holder">
      <button onClick={() => handleReaction('Like', null)} data-testid="like-button">Like</button>
      <button onClick={setShowReactions} data-testid="show-reactions">Show Reactions</button>
    </div>
  )
}));
...
describe('Reply Component', () => {
  ...
  it('renders reply content correctly', () => {
    ...
    expect(screen.getByTestId('text-viewer')).toHaveTextContent('This is a reply to a comment');
  ...
  it('handles delete action correctly', () => {
    ...
    expect(mockHandleDeleteReplyToComment).toHaveBeenCalledWith(mockCommentId, mockReply.id);
  });
