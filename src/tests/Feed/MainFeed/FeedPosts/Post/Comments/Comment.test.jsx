    ...
    <div data-testid="dropdown-menu">
      ...
      <ul>
        {menuItems.map((item, index) => (
          <li key={index} onClick={item.onClick} data-testid={`menu-item-${index}`}>
            <item.icon />
            {item.text}
          </li>
        ))}
...
vi.mock('../../../../../../pages/Feed/MainFeed/FeedPosts/ReactionModal/ReactionsModal', () => ({
  default: ({ setShowLikes }) => (
    <div data-testid="reactions-modal">
      <button onClick={setShowLikes} data-testid="close-reactions">Close</button>
    </div>
  )
}));

vi.mock('../../../../../../pages/Feed/MainFeed/FeedPosts/Post/Comments/ActivitiesHolder', () => ({
  default: ({ currentReaction, reactions, handleReaction, setShowReactions, replies, setShowReplies }) => (
    <div data-testid="activities-holder">
      <button onClick={() => handleReaction('Like', null)} data-testid="react-button">React</button>
      <button onClick={setShowReactions} data-testid="show-reactions">Show Reactions ({
...
vi.mock('../../../../../../pages/Feed/MainFeed/FeedPosts/Post/Comments/CommentThreadWrapper', () => ({
  default: ({ children }) => <div data-testid="thread-wrapper">{children}</div>
}));
...
vi.mock('../../../../../../pages/Feed/MainFeed/FeedPosts/Post/Comments/AddForm', () => ({
  default: ({ handleAddFunction, initialText, close, type }) => (
    <div data-testid="add-form">
      ...
      <button onClick={() => handleAddFunction('Edited text', [])} data-testid="submit-edit">Submit</button>
      <button onClick={close} data-testid="cancel-edit">Cancel</button>
    </div>
  )
}));

vi.mock('../../../../../../pages/Feed/MainFeed/FeedPosts/Post/Comments/ReplyContainer', () => ({
  default: ({ commentId }) => <div data-testid="reply-container">Replies for comment {commentId}</div>
}));
...
describe('Comment Component', () => {
  ...
  it('renders comment content correctly', () => {
    ...
    expect(screen.getByTestId('text-viewer').textContent).toBe(mockComment.content);
  ...
  it('switches to edit mode when edit option is clicked', () => {
    const { getByText, getByTestId } = customRender(<Comment comment={mockOwnComment} />);
  ...
  it('submits edited comment correctly', async () => {
    ...
    await waitFor(() => {
      expect(mockHandleEditComment).toHaveBeenCalledWith(mockOwnComment.id, 'Edited text', []);
    });
  ...
  it('shows replies when show replies is clicked', () => {
    ...
    expect(screen.getByText(`Replies for comment ${mockComment.id}`)).toBeInTheDocument();
  });
  ...
  it('handles reaction on comment correctly', async () => {
    ...
    expect(mockHandleReactOnComment).toHaveBeenCalledWith(mockComment.id, 'Like', null);
  });
});