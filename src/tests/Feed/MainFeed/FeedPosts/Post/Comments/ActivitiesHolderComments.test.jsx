...
vi.mock('../../../../../../pages/Feed/GenericComponents/reactionIcons', () => ({
  default: {
    Like: { Icon: () => <div data-testid="like-icon">👍</div>, color: 'blue', label: 'Like' },
    Love: { Icon: () => <div data-testid="love-icon">❤️</div>, color: 'red', label: 'Love' },
    Celebrate: { Icon: () => <div data-testid="celebrate-icon">🎉</div>, color: 'orange', label: 'Celebrate' },
    Support: { Icon: () => <div data-testid="support-icon">🙌</div>, color: 'purple', label: 'Support' },
    Insightful: { Icon: () => <div data-testid="insightful-icon">💡</div>, color: 'green', label: 'Insightful' }
  }
}));

vi.mock('../../../../../../pages/Feed/GenericComponents/ReactionPicker', () => ({
  default: ({ children, onSelectReaction }) => (
    <div data-testid="reaction-picker" onClick={() => onSelectReaction('Love')}>
      {children}
    </div>
  )
}));
...
describe('ActivitiesHolder Component', () => {
  ...
  it('handles error during reaction submission gracefully', async () => {
    ...
    const errorReactionHandler = vi.fn().mockRejectedValue(new Error('Failed to react'));
