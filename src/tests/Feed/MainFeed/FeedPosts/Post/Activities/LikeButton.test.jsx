...
// Mock the CSS import
vi.mock(
  "../../../../../../pages/Feed/MainFeed/FeedPosts/Post/Activities/LikeButton.css",
  () => ({}),
);

// Create a mock function that resolves immediately
const mockHandleReactOnPost = vi
  .fn()
...
vi.mock("react", async () => {
  ...
  return {
    ...
    useState: (initialValue) => {
      ...
      if (
        initialValue === false &&
        (mockSetIsLoading.mock.calls.length === 0 ||
          mockSetIsReacted.mock.calls.length > 0)
      ) {
...
describe("LikeButton Component", () => {
  beforeEach(() => {
    ...
    mockSetIsLoading.mockImplementation((value) => {
      isLoadingState =
        typeof value === "function" ? value(isLoadingState) : value;
    });

    mockSetIsReacted.mockImplementation((value) => {
      isReactedState =
        typeof value === "function" ? value(isReactedState) : value;
    });
