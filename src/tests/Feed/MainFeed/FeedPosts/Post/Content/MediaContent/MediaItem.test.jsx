...
vi.mock('../../../../../../../pages/Feed/MainFeed/FeedPosts/Post/Content/MediaContent/PdfViewer', () => ({
  default: ({ url }) => <div data-testid="pdf-viewer" data-url={url}>PDF Viewer</div>
}));
...
describe('MediaItem Component', () => {
  ...
  it('renders a PDF viewer when given a document URL', () => {
    ...
    expect(pdfViewer).toHaveAttribute('data-url', 'https://example.com/file.document');
  });
  ...
  it('does not show play overlay when video is not disabled', () => {
    ...
    const overlays = document.querySelectorAll('div.absolute.inset-0.bg-black\\/20');
  ...
  it('uses memoized mediaType calculation', () => {
    const { rerender } = renderComponent({ url: 'https://example.com/image.jpg' });
