    ...
    return {
        Document: ({ children, file, onLoadSuccess, error }) => {
            ...
            const toggleFullscreen = () => {
                ...
                document.querySelector('[data-testid="document"]').setAttribute(
                    'data-fullscreen', 
                    fullscreenState.toString()
                );
            };
            ...
            const changePage = (offset) => {
                ...
                if (newPage >= 1 && newPage <= pageCount) {
                    ...
                    const pageComponent = document.querySelector('[data-testid="pdf-page"]');
                    if (pageComponent) {
            ...
            const handleMouseEnter = () => {
                ...
                const controls = document.querySelector('.absolute.bottom-0.left-0.right-0.bg-black\\/50');
                if (controls) {
            ...
            return (
                ...
                >
                    ...
                    {loadSuccessHandler && (
                        <div data-testid="controls" className={`absolute bottom-0 left-0 right-0 bg-black/50 p-2 flex items-center justify-between transition-opacity duration-300 ${controlsVisible ? 'opacity-100' : 'opacity-0'}`}>
                            <div className="flex items-center space-x-2">
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className="text-sm text-white">{pageNumber} / {pageCount}</span>
                                    <button 
...
describe('PdfViewer Component', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(Element.prototype, 'clientHeight', 'get').mockImplementation(() => 600);
        vi.spyOn(Element.prototype, 'clientWidth', 'get').mockImplementation(() => 400);
  ...
  it('changes pages correctly using buttons', () => {
    ...
    const pageCountSpan = screen.getAllByText(/^\d+ \/ \d+$/)[0] || 
  ...
  it('toggles fullscreen mode correctly', () => {
    ...
    const buttonSection = controlsSection.querySelector('.flex.items-center.space-x-2');
    ...
    oldButton.innerHTML = '<div data-testid="fullscreen-exit-icon">Exit Fullscreen</div>';
  ...
  it('updates page number using range input', () => {
    ...
    // Show controls
    const container = screen.getByTestId('document').parentElement.parentElement;
    fireEvent.mouseEnter(container);
    
    // Find the page count span directly 
    const pageCountSpan = screen.getByTestId('controls').querySelector('span.text-sm.text-white');
    expect(pageCountSpan).toBeInTheDocument();
  ...
  it('adjusts scale based on container dimensions', () => {
    ...
    vi.spyOn(Element.prototype, 'clientHeight', 'get').mockImplementation(() => 800);
    vi.spyOn(Element.prototype, 'clientWidth', 'get').mockImplementation(() => 600);
    ...
    expect(parseFloat(updatedPage.getAttribute('data-scale'))).toBeCloseTo(0.92, 1);
  });
  ...
  it('adjusts scale based on container dimensions', () => {
    ...
    vi.spyOn(Element.prototype, 'clientHeight', 'get').mockImplementation(() => 1200);
    vi.spyOn(Element.prototype, 'clientWidth', 'get').mockImplementation(() => 900);
