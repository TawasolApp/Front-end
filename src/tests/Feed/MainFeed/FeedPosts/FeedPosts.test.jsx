...
describe('FeedPosts Component', () => {
  ...
  it('renders nothing when posts is null or undefined', () => {
    ...
    const postContainersAfterRerender = screen.queryAllByTestId('post-container');
  ...
  it('applies ref to the last post', () => {
    const mockRef = vi.fn();
    const { container } = render(<FeedPosts {...defaultProps} lastPostRef={mockRef} />);
  ...
  it('passes correct props to PostContainer', () => {
    ...
    expect(postContainerProps[0]).toEqual(expect.objectContaining({
    ...
    expect(postContainerProps[1].post).toEqual({ id: '2', content: 'Test post 2' });
    
    // And the third post
    expect(postContainerProps[2].post).toEqual({ id: '3', content: 'Test post 3' });
  });
});