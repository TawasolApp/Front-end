...
describe("FeedContainer", () => {
  ...
  it("has the correct responsive layout structure", () => {
    ...
    // LG screens layout
    expect(
      container.querySelector(".hidden.lg\\:flex.xl\\:hidden"),
    ).toBeTruthy();

    // MD screens layout
    expect(
      container.querySelector(".hidden.md\\:flex.lg\\:hidden"),
    ).toBeTruthy();

    // SM screens layout
    expect(
      container.querySelector(".hidden.sm\\:block.md\\:hidden"),
    ).toBeTruthy();
  ...
  it("has correct layout for extra large screens", () => {
    ...
    expect(
      xlLayout.querySelectorAll('[data-testid="left-sidebar"]').length,
    ).toBe(1);
    expect(xlLayout.querySelectorAll('[data-testid="main-feed"]').length).toBe(
      1,
    );
    expect(
      xlLayout.querySelectorAll('[data-testid="right-sidebar"]').length,
    ).toBe(1);
  });
  ...
  it("has correct layout for medium screens (no right sidebar)", () => {
    ...
    expect(
      mdLayout.querySelectorAll('[data-testid="left-sidebar"]').length,
    ).toBe(1);
    expect(mdLayout.querySelectorAll('[data-testid="main-feed"]').length).toBe(
      1,
    );
    expect(
      mdLayout.querySelectorAll('[data-testid="right-sidebar"]').length,
    ).toBe(0);
  });
});
