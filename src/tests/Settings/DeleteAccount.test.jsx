...
describe("DeleteAccount Component", () => {
  ...
  it("submits the form and navigates to sign in page on successful account deletion", async () => {
    ...
    await waitFor(() => {
      expect(axiosInstance.delete).toHaveBeenCalledWith("/users/delete-account");
  ...
  it("handles errors during account deletion", async () => {
    ...
    axiosInstance.delete.mockRejectedValueOnce(new Error("Delete account failed"));
    ...
    await waitFor(() => {
      expect(axiosInstance.delete).toHaveBeenCalledWith("/users/delete-account");
      expect(consoleErrorSpy).toHaveBeenCalledWith("Error deleting account:", expect.any(Error));
