...
vi.mock(
  "../pages/Feed/MainFeed/FeedPosts/PostCard/Comments/ActivitiesHolder",
  () => ({
    default: ({
      initReactValue,
      reactions,
      ...
      <div data-testid="activities-holder">
        ...
        <button data-testid="show-reactions-button" onClick={setShowReactions}>
          Show Reactions
        </button>
...
vi.mock(
  "../pages/Feed/MainFeed/FeedPosts/PostCard/Comments/CommentThreadWrapper",
  () => ({
    default: ({ children }) => (
      <div data-testid="comment-thread-wrapper">{children}</div>
    ),
  }),
);
  ...
  () => ({
    default: ({ APIURL, setShowLikes }) => (
      <div data-testid="reactions-modal" data-api-url={APIURL}>
        <button data-testid="close-reactions" onClick={setShowLikes}>
          Close
        </button>
      </div>
    ),
...
vi.mock("../pages/Feed/MainFeed/FeedPosts/PostCard/Comments/AddForm", () => ({
  default: ({ handleAddFunction, initialText, close, type }) => (
    <div data-testid="add-form" data-type={type}>
      ...
      <button data-testid="cancel-edit" onClick={close}>
        Cancel
      </button>
    </div>
  ),
...
describe("Comment Component", () => {
  ...
  it("shows reactions modal when reactions count is clicked", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("reactions-modal")).toHaveAttribute(
        "data-api-url",
        "/posts/reactions/123",
      );
    });
