...
describe("PostCardHeader Component", () => {
  ...
  it("passes correct props to ActorHeader", () => {
    ...
    expect(screen.getByTestId("author-id")).toHaveTextContent(
      defaultProps.authorId,
    );
    expect(screen.getByTestId("author-name")).toHaveTextContent(
      defaultProps.authorName,
    );
    expect(screen.getByTestId("author-bio")).toHaveTextContent(
      defaultProps.authorBio,
    );
    expect(screen.getByTestId("author-picture")).toHaveAttribute(
      "src",
      defaultProps.authorPicture,
    );
  ...
  it("renders with correct structure and styling classes", () => {
    ...
    const actorHeaderContainer = container.querySelector(
      ".pr-16.pl-3.pt-3.mb-2",
    );
