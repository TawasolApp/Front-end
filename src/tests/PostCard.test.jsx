...
vi.mock('../pages/Feed/MainFeed/FeedPosts/PostCard/Header/PostCardHeader', () => ({
  default: (props) => (
    <div data-testid="post-card-header">
      ...
      <button 
        ...
        onClick={() => props.menuItems.length > 0 && props.menuItems[0].onClick()}
      >
        Menu
      </button>
...
vi.mock('../pages/Feed/MainFeed/FeedPosts/PostCard/Content/PostContent', () => ({
  default: (props) => (
...
vi.mock('../pages/Feed/MainFeed/FeedPosts/PostCard/Metrics/EngagementMetrics', () => ({
  default: (props) => (
...
vi.mock('../pages/Feed/MainFeed/FeedPosts/PostCard/Activities/ActivitiesHolder', () => ({
  default: (props) => (
...
vi.mock('../pages/Feed/MainFeed/FeedPosts/PostCard/Comments/CommentsContainer', () => ({
  default: (props) => (
...
describe('PostCard Component', () => {
  ...
  it('displays the post content correctly', () => {
    ...
    expect(screen.getByTestId('content-text')).toHaveTextContent(mockPost.content);
  });
  ...
  it('toggles comments visibility when show comments is clicked', () => {
    ...
    expect(screen.getByTestId('comments-post-id')).toHaveTextContent(mockPost.id);
  });
  ...
  it('shows likes modal when show likes is clicked', () => {
    ...
    expect(screen.getByTestId('reactions-api-url')).toHaveTextContent(`/posts/reactions/${mockPost.id}`);
  ...
  it('shows edit modal when edit post is clicked', async () => {
    ...
    const originalPostCardHeader = vi.importMock('../pages/Feed/MainFeed/FeedPosts/PostCard/Header/PostCardHeader').default;
    
    vi.doMock('../pages/Feed/MainFeed/FeedPosts/PostCard/Header/PostCardHeader', () => ({
      default: (props) => (
        <div data-testid="post-card-header">
          {props.menuItems && props.menuItems.map((item, index) => (
            <button 
  ...
  it('toggles saved status when save post is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('menu-item-0')).toHaveTextContent('Unsave post');
    });
  ...
  it('copies link to clipboard when copy link option is clicked', async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(`http://localhost:5173/posts/${mockPost.id}`);
  });
