...
const TextViewer = ({ 
  ...
  const processText = () => {
    ...
    while ((match = mentionRegex.exec(text)) !== null) {
      ...
      const userId = mentionIndex < taggedUsers.length ? taggedUsers[mentionIndex] : null;
      processedContent.push({
        type: "mention",
  ...
  const plainText = processedContent.map(segment => {
  ...
  const renderContent = () => {
    if (!needsTruncation || isExpanded) {
      ...
      for (let i = 0; i < processedContent.length; i++) {
        ...
        const segmentText = segment.type === "url" ? "External Link" : segment.content;
        ...
        for (let j = 0; j < segmentLines.length; j++) {
          ...
          if (charCount + line.length <= maxChars && lineCount < maxLines) {
            ...
            if (segment.type === "text") {
              currentLine.push({...segment, content: j === 0 ? line : (j > 0 ? line : segment.content)});
            } else {
            ...
            if (segment.type === "text") {
              currentLine.push({...segment, content: line.substring(0, remainingChars)});
              charCount += remainingChars;
              currentLineChars += remainingChars;
            } else {
              // For non-text segments (mentions, URLs) we'll add them fully if we have space
              if (segment.type === "url" && remainingChars >= 12) { // "External Link" length
                currentLine.push(segment);
                charCount += 12;
                currentLineChars += 12;
              } else if (segment.type === "mention" && remainingChars >= segment.content.length) {
                currentLine.push(segment);
      ...
      return (
        <div className="text-textContent whitespace-pre-wrap break-words text-sm font-normal">
          {truncatedContent.map((line, lineIdx) => (
            <div key={lineIdx}>
              {line.map((segment, segIdx) => renderSegment(segment, `${lineIdx}-${segIdx}`))}
