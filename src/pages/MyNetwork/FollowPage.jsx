...
const FollowPage = () => {
  ...
  const lastElementRef = useCallback(
    (node) => {
      ...
      observer.current = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && hasMore[activeTab] && !isFetching.current) {
  ...
  const handleUnfollow = async () => {
    ...
    try {
      await axiosInstance.delete(`/connections/unfollow/${userToUnfollow.userId}`);
  ...
  const currentCount = activeTab === "following" ? following.length : followers.length;
              ...
              <div className="divide-y divide-cardBorder">
                {currentData.map((user, index) => (
                  <div
                    ...
                    ref={index === currentData.length - 1 ? lastElementRef : null}
                    className="py-4"
                  >
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                      <div className="flex items-center gap-3 w-full sm:w-3/4 overflow-hidden">
                        ...
                        <div className="min-w-0">
                          <h3 className="font-bold text-textHeavyTitle truncate hover:underline"
