describe("adds, updates, deletes work experience", () => {
  ...
  it("interacts with work experience", () => {
    ...
    cy.get(".text-sm.text-companyheader.font-medium")
      .should("contain.text", "Tawasol")
      .and("contain.text", "internship");
    cy.contains("Jul 2024").should("be.visible");

    // add work experience
    cy.get(
      ":nth-child(4) > .p-6 > .justify-between > .gap-0 > :nth-child(1)",
    ).click();
    ...
    // assert existence
    cy.get(
      ":nth-child(2) > .bg-boxbackground > .pr-8 > .flex > .break-all > .text-lg",
    ).should("include.text", "Consultant");
    cy.get(".text-sm.text-companyheader.font-medium")
      .should("contain.text", "Apple")
      .and("contain.text", "contract");
