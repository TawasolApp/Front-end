describe("adds, updates, deletes cover photo", () => {
  ...
  it("interacts with cover photo", () => {
    ...
    cy.get('img[alt="Cover"]')
      .invoke("attr", "src")
      .should("include", "src/assets/images/defaultCoverPhoto.png");
    cy.get('img[alt="Cover"]').invoke("attr", "src").as("originalSrc");

    // add cover photo
    cy.get(".w-5").click();
    cy.wait(2000);
    cy.get('[data-testid="file-input"]').attachFile("imgs/polar_bear.png", {
      force: true,
    });
    cy.wait(5000);
    ...
    // get new image
    cy.get('img[alt="Cover"]').invoke("attr", "src").as("newSrc");

    // update cover photo
    cy.get(
      ".w-full.shadow > :nth-child(1) > .relative > .absolute > .w-8",
    ).click();
    cy.wait(2000);
    cy.get('[data-testid="file-input"]').attachFile("imgs/orca.png", {
      force: true,
    });
    cy.wait(5000);
    cy.get(".mt-5 > .text-white").click();
    ...
    // get newer image
    cy.get('img[alt="Cover"]').invoke("attr", "src").as("newerSrc");
